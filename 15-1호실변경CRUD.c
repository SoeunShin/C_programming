...
void editRoom(struct Record* p[], int n){
    ...
    while(1){
      ...
      int cnt = 0;
      if(rp->fm == 'F'){ // 여학생이면 
        if(newroom == rp->room) // 이전 호실 입력 
          break;
        else if(newroom >= 201 && newroom <= 215) // 남학생 층 입력 
          printf("배정불가(층 오류)\n");
        else if(newroom >= 101 && newroom <= 115){ // 이미 2명 입실함 
          for(i=0;i<n;i++){ // 해당 호실 인원 세기 
            if(p[i]->room == newroom) cnt++;
          }
          if(cnt > 1) printf("배정불가(인원초과)\n");
          else break;
        }
        else // 존재하지 않는 호실 
          printf("배정불가(호실 오류)\n");
        ...
        if(newroom == rp->room) // 이전 호실 입력 
          break;
        else if(newroom >= 101 && newroom <= 115)
          printf("배정불가(층 오류)\n");
        else if(newroom >= 201 && newroom <= 215){ // 이미 2명 입실함 
          for(i=0;i<n;i++){ // 해당 호실 인원 세기 
            if(p[i]->room == newroom) cnt++;
          }
          if(cnt > 1) printf("배정불가(인원초과)\n");
